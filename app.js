...
function classifyAndSaveEvent(text) {
  ...
  if (text.includes("weaker since morning") && text.includes("skipped breakfast")) {
    agentResponse = "I have recorded this as a new event. I'll mark it as needing attention. Would you like me to set a reminder for you to check in on her later?";
  } else if (eventType === 'symptom') {
    agentResponse = `I have logged the symptom: "${summary}". I have flagged this as needing attention. Are there any other symptoms or recent changes you would like to note?`;
  } else if (eventType === 'medication' && attentionLevel === 'high') {
    agentResponse = `Logged medication incident: "${summary}". I've marked this with a high attention level. Did she take it later or experience any discomfort?`;
  } else {
    agentResponse = `I have successfully logged this care note under ${eventType.toUpperCase()} in your history.`;
  }
...
function generateDoctorSummary() {
  ...
  let summaryOutput = `Based on available information, here is a summary for the ${rangeLabel}: 
...
${questionsText}`;
  ...
  if (filteredEvents.length === 0) {
    summaryOutput = `Based on available information, here is a summary for the ${rangeLabel}: 
