// Care Companion Agent - Application Logic
...
function processUserInput() {
  ...
  console.log(`[Care Companion Agent] User input submitted: "${text}"`);
...
function handleAgentResponse(text) {
  console.log(`[Care Companion Agent] Invoking handleAgentResponse for input: "${text}"`);
  ...
  if (
    ...
    console.warn(`[Care Companion Agent] Safety check triggered: Diagnostic query detected. Blocking response.`);
    appendMessage(
      'agent',
  ...
  if (lowerText.includes('generate summary') || lowerText.includes('doctor summary') || lowerText.includes('appointment summary')) {
    console.log(`[Care Companion Agent] Doctor summary intent detected.`);
  ...
  console.log(`[Care Companion Agent] Care logging intent detected. Classifying event...`);
  classifyAndSaveEvent(text);
}

function classifyAndSaveEvent(text) {
  console.log(`[Care Companion Agent] Classifying event input: "${text}"`);
  const lowerText = text.toLowerCase();
  let eventType = 'other';
  ...
  console.log(`[Care Companion Agent] Event successfully classified:`, newEvent);
  ...
  console.log(`[Care Companion Agent] Sending agent response: "${agentResponse}"`);
...
function generateDoctorSummary() {
  ...
  console.log(`[Care Companion Agent] Generating doctor summary for range (days): ${rangeDays}`);
  const now = new Date();
